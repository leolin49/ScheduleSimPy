...
def generate_pod_yaml(
    ...
    for k, v in annotations.items():
        ...
        if v is not None:
            workload_yaml["metadata"]["annotations"][
                k
            ] = v  # e.g., {"alibabacloud.com/gpu-index":"2-3-4"}
...
def output_pod(dfp, outfile="pod.yaml", node_select=False):
    ...
    for index, row in dfp.iterrows():
        ...
        if int(row["num_gpu"]) != 0:
            if node_select:
                annotations[DeviceIndex] = (
                    row["gpu_index"] if type(row["gpu_index"]) == str else ""
                )
            if "gpu_milli" not in row:
                annotations[ResourceName] = 1000
            else:
                annotations[ResourceName] = (
                    "%d" % (int(row["gpu_milli"]))
                    if 0 < row["gpu_milli"] <= 1000
                    else "1000"
                    if row["gpu_milli"] > 1000
                    else "0"
                )
        ...
        pod_yaml = generate_pod_yaml(
            workload_name=workload_name,
            container_requests=container_requests,
            container_limits=container_limits,
            node_selector_node_ip=host_node_ip,
