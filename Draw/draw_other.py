...
for file in files:
    with open(os.path.join(directory, file), 'r') as f:
        ...
        data_by_baseline[baseline]["avg_task_make_span"].append(float(data["avg_task_make_span"]))
        data_by_baseline[baseline]["avg_cpu_utilization"].append(float(data["avg_cpu_utilization"].strip('%')))
        data_by_baseline[baseline]["avg_mem_utilization"].append(float(data["avg_mem_utilization"].strip('%')))
        data_by_baseline[baseline]["avg_gpu_utilization"].append(float(data["avg_gpu_utilization"].strip('%')))

# 指标名称和获取对应数据的键
metrics_names = ['Task Make Span', 'CPU Utilization', 'Memory Utilization', 'GPU Utilization']
t = ['Task Make Span', 'CPU Utilization', 'Memory Utilization']
metrics_keys = ['avg_task_make_span', 'avg_cpu_utilization', 'avg_mem_utilization', 'avg_gpu_utilization']
...
for metric_name, metric_key, metrics_ylim in zip(metrics_names, metrics_keys, metrics_ylims):
    if metric_name in t:
        continue
    ...
    index = np.arange(num_params) * (bar_width * len(data_by_baseline) + gap_between_groups)

    # 为每个 baseline 绘制柱状图
    colors = ['brown', 'green', 'red', 'blue', 'orange', 'purple']
    for idx, (baseline, data) in enumerate(data_by_baseline.items()):
        plt.bar(index + idx * bar_width, data[metric_key], bar_width, color=colors[idx], label=baseline.upper(), hatch='', alpha=1)

    # 设置标签和标题
    plt.xlabel('Parameter Combination (|N|, |C|)', fontsize=15)
    plt.ylabel(f'{metric_name} (%)', fontsize=15)
    plt.title(f'Comparison of {metric_name} Across Baselines for Different Parameters', fontsize=15)

    # 设置 x 轴的刻度
    plt.xticks(index + (len(data_by_baseline) * bar_width) / 2, next(iter(data_by_baseline.values()))["params"],
